...
#TODO - should we have r and σ for aerosol, droplets and ice?
...
function parcel_model(dY, Y, p, t)
    ...
    dqᵢ_dt_l2i = dqᵢ_dt_imm
    dqᵢ_dt_v2i = dqᵢ_dt_dep + dqᵢ_dt_ds
    # special case for the Jensen example
    if Jensen
        dNₐ_dt -= dNᵢ_dt_hom
        dqᵢ_dt_v2i += dqᵢ_dt_hom
    else
        dNₗ_dt -= dNᵢ_dt_hom
        dqᵢ_dt_l2i += dqᵢ_dt_hom
    end
...
The parcel parameters struct comes with default values that can be overwritten:
 ...
 - Jensen - boolean to flag the special case of running example Jensen_et_al_2022
 - r_nuc - assumed size of nucleating ice crystals. Default value is 5e-11 [m]
 - σ - assumed standard deviation for the aerosol particle distribution. Defaul value is 2
"""
function run_parcel(IC, t_0, t_end, pp)
    ...
    elseif pp.size_distribution == "Lognormal"
        distr = Lognormal{FT}(pp.r_nuc, pp.σ)
    else
        throw("Unrecognized size_distribution")
    end
    ...
    elseif pp.deposition == "ActivityBased"
        dep_params = ActivityBased{FT}(pp.tps, pp.aerosol, pp.r_nuc)
    ...
    if pp.heterogeneous == "None"
        imm_params = Empty{FT}()
    elseif pp.heterogeneous == "ImmersionFreezing"
    ...
    if pp.homogeneous == "None"
        hom_params = Empty{FT}()
    elseif pp.homogeneous == "ActivityBased"
        hom_params = WaterAct{FT}(pp.tps, pp.ips, pp.Jensen)
    ...
    println("Growth modes: ", pp.growth_modes)
    if "Condensation" in pp.growth_modes
        ce_params = CondParams{FT}(pp.aps, pp.tps)
    else
        ce_params = Empty{FT}()
    end
    if "Deposition" in pp.growth_modes
